...
class Engine:
    ...
    def text_output(self):
        output =  "Engine Name: "+self.name
        output += "\n"
        output += "\nThrust: "+str(self.thrust)+" N"
        output += "\nThrust Vac: "+str(self.thrust_vac)+" N"
        output += "\nIsp: "+str(self.Isp)+" s"
        output += "\nIsp Vac: "+str(self.Isp_vac)+" s"
        output += "\nMdot: "+str(self.mdot)+" kg/s"
        output += "\nMixture Ratio: "+str(self.MR)+":1"
        output += "\n"
        output += "\nGeometry:"
        output += "\nAc, Chamber Area: "+str(self.Ac)+" m^2"
        output += "\nRc, Chamber Radius: "+str(self.Rc)+" m"
        output += "\nAt, Throat Area: "+str(self.At)+" m^2"
        output += "\nRt, Throat Radius: "+str(self.Rt)+" m"
        output += "\nAe, Exit Area: "+str(self.Ae)+" m^2"
        output += "\nRe, Exit Radius: "+str(self.Re)+" m"
        output += "\nRn, Radius Leaving Throat: "+str(self.Rn)+" m"
        output += "\nEa, expansion area ratio (Ae/At): "+str(self.expansion_area_ratio)+":1"
        output += "\nEc, contraction area ratio (Ac/Ae): "+str(self.contraction_area_ratio)+":1"
        output += "\nThetac, contraction angle: "+str(self.contraction_angle)+" degrees"
        output += "\nL*: "+str(self.lstar)+" m"
        output += "\nVc, chamber volume: "+str(self.Vc)+" m"
        output += "\nlcyl, cylindrical section of combustion chamber: "+str(self.lcyl)+" m"
        output += "\nlength of nozzle: "+str(self.ln)+" m"
        return output
